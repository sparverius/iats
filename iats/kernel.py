...
class ATSKernel(Kernel):
    implementation = 'jupyter_c_kernel'
    implementation_version = '1.0'
    language = 'ats'
    language_version = 'temptory'
    language_info = {'name': 'ats',
                     'mimetype': 'text/plain',
                     'file_extension': '.dats'}
    banner = "ATS kernel.\n" \
    ...
    def __init__(self, *args, **kwargs):
        super(ATSKernel, self).__init__(*args, **kwargs)
        self.files = []
    ...
    def compile_with_gcc(self, source_filename, binary_filename, cflags=None, ldflags=None):
        ...
        args = ['tempacc', '-DATS_MEMALLOC_LIBC', source_filename] + cflags + ['-o', binary_filename] + ldflags
    ...
    def do_execute(self, code, silent, store_history=True,
        ...
        with self.new_temp_file(suffix='.dats') as source_file:
            ...
            with self.new_temp_file(suffix='.out') as binary_file:
                ...
                if p.returncode != 0:  # Compilation failed
                    self._write_to_stderr(
                            "[ATS kernel] GCC exited with code {}, the executable will not be executed".format(
                                    p.returncode))
        ...
        if p.returncode != 0:
            self._write_to_stderr("[ATS kernel] Executable exited with code {}".format(p.returncode))
